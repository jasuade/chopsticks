...
func (c *SDLController) EndGame() {
	c.Resources.fontMessage.Close()
	sdl.Quit()
	ttf.Quit()
	c.window.Destroy()
	c.Renderer.Destroy()
...
func (c *SDLController) PrintBackground() error {
	dstRect := &sdl.Rect{X: 0, Y: 0, W: 1000, H: 800}

	if err := c.Renderer.Copy(c.Resources.backgroundTexture, nil, dstRect); err != nil {
		return err
	}
	return nil
}

func (c *SDLController) PrintTitle() error {
	dstRect := &sdl.Rect{X: 100, Y: 5, W: 800, H: 100}

	if err := c.Renderer.Copy(c.Resources.titleTexture, nil, dstRect); err != nil {
		return err
	}
	return nil
}

func (c *SDLController) PrintMessage(text string) error {

	surface, err := c.Resources.fontMessage.RenderUTF8Solid(text, sdl.Color{
		R: 255,
		G: 255,
		B: 255,
		A: 255,
	...
	defer surface.Free()
	messageTexture, err := c.Renderer.CreateTextureFromSurface(surface)
	dstRect := &sdl.Rect{X: (1000 - 700) / 2, Y: 430, W: 700, H: 30}

	if err := c.Renderer.Copy(messageTexture, nil, dstRect); err != nil {
		return err
	}
	return nil
}

func (c *SDLController) PrintPlayers(players []game.PlayerI) error {
	...
	for _, player := range players {
		...
		textureLeftHand := c.Resources.handTextures[leftHand]
		textureRightHand := c.Resources.handTextures[rightHand]
		...
		if err := c.Renderer.CopyEx(textureLeftHand, srcRectLeft, dstRect, angle, nil, 0); err != nil {
			return err
		}
		...
		if err := c.Renderer.CopyEx(textureRightHand, srcRectRight, dstRect, angle, nil, 5); err != nil {
			return err
		}
		angle += 180
		positionY -= 400
